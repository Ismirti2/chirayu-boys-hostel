...
export default function AdminLogin() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const success = login("admin", credentials)
      if (success) {
        router.push("/admin-dashboard")
      } else {
        setError(
          "Invalid credentials. Try Username: admin, Password: chirayu123"
        )
      }
      setLoading(false)
    }, 800)
      ...
      >
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-8">
          ...
          >
            ...
            <Link href="/student-login" className="block text-slate-600 dark:text-slate-400 hover:text-chirayu-blue dark:hover:text-chirayu-teal">
              Student Login?
            </Link>
          </motion.div>
          ...
          >
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400">
              <span className="font-medium">Password:</span> chirayu123
            </p>
          </motion.div>
        </div>
      </motion.div>
